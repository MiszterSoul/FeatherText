...
const icon = (...nodes) => `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${nodes.join('')}</svg>`;
const pathNode = (d) => `<path d="${d}"></path>`;
const lineNode = (x1, y1, x2, y2) => `<line x1="${x1}" y1="${y1}" x2="${x2}" y2="${y2}"></line>`;
const rectNode = (x, y, width, height, rx) => `<rect x="${x}" y="${y}" width="${width}" height="${height}" rx="${rx}" ry="${rx}"></rect>`;
const circleNode = (cx, cy, r) => `<circle cx="${cx}" cy="${cy}" r="${r}"></circle>`;
const polylineNode = (points) => `<polyline points="${points}"></polyline>`;
...
const button = (iconName, tip, action) => Object.assign({ icon: iconMarkup[iconName], tip }, action);
...
export default class FeatherText {
	constructor(element, config) {
		...
		this._scheduleFrame = typeof window !== 'undefined' && typeof window.requestAnimationFrame === 'function'
			? window.requestAnimationFrame.bind(window)
			: function (callback) { return setTimeout(callback, 16); };
		this._cancelFrame = typeof window !== 'undefined' && typeof window.cancelAnimationFrame === 'function'
	...
	_attachThemeListener() {
		...
		this._themeListener = (event) => {
			const resolved = event && typeof event.matches === 'boolean'
				? (event.matches ? 'dark' : 'light')
				: this.resolveThemeName('auto');
			const root = document.documentElement;
			root.setAttribute('data-theme', resolved);
			Object.entries(themes[resolved]).forEach(([k, v]) => root.style.setProperty(`--feather-${k}`, v));
		};
	...
	exec(name, def) {
		if (def) {
			...
			else if (def.cmd) { this.execCommand(def.cmd, def.value || null); this.pushHistory(); }
		}
	...
	applySourcePreferences() {
		...
		this.sourceWrap.style.setProperty('--feather-source-indent', indentUnit.replace(/ /g, '\u00a0'));
	...
	handleToolbarKeydown(e) {
		...
		const focusables = Array.from(container.querySelectorAll('button:not([disabled]), [role="button"], select:not([disabled])'))
	...
	handlePaste(e) {
		...
		if (!pasteContent) {
			const mode = this.config.pasteMode === 'auto'
				? (this.config.sanitizePaste ? 'text' : 'html')
	...
	pushHistory() { const html = this.editor.innerHTML; this.history = this.history.slice(0, this.historyIndex + 1); this.history.push(html); this.historyIndex++; if (this.history.length > this.config.historyLimit) { this.history.shift(); this.historyIndex--; } }

	updateSourceControlsState() { if (this.sourceHeader) this.sourceHeader.style.display = this.isSource ? 'flex' : 'none'; }
	...
	selectBracketPair() {
		const ta = this.source; if (!ta) return; const text = ta.value; let pos = ta.selectionStart; const br = '()[]{}<>';
		if (pos > 0 && !br.includes(text[pos]) && br.includes(text[pos - 1])) pos = pos - 1; const ch = text[pos]; const pairs = Object.assign({ '<': '>' }, SOURCE_BRACKET_PAIRS); const rev = { ')': '(', ']': '[', '}': '{', '>': '<' };
	...
	// History
	undo() { if (this.historyIndex > 0) { this.historyIndex--; const html = this.history[this.historyIndex] || ''; this.editor.innerHTML = html; this.element.value = html; this.flushCountsUpdate(); } }
	...
	addButton(name, def) { buttons[name] = Object.assign({}, def, { icon: def && def.icon ? def.icon : iconMarkup.code, tip: def && (def.tip || def.tooltip || name) }); const btn = this.createToolbarButton(name); if (btn) { const groups = this.toolbar.querySelectorAll('.feather-group'); const last = groups[groups.length - 1]; if (last) last.appendChild(btn); } }
	...
	async pasteAction() {
		try {
			...
			if (!text) return;
			if (this.isSource) {
				const ta = this.source; const s = ta.selectionStart, e2 = ta.selectionEnd; ta.setRangeText(text, s, e2, 'end'); this.scheduleSourceRefresh(true);
			} else {
	...
	clearFormatting() { this.execCommand('removeFormat'); this.updateHistory(); }
	...
	static init(selector, cfg) { const nodes = document.querySelectorAll(selector); const arr = []; nodes.forEach(n => arr.push(new FeatherText(n, cfg))); return arr; }
}

export { themes, buttons, iconMarkup };
export const version = (typeof __FEATHER_VERSION__ !== 'undefined' ? __FEATHER_VERSION__ : '0.0.0-dev');
